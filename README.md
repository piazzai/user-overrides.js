...
3. Copy arkenfox's `user.js`, `updater.sh`, and `prefsCleaner.sh` to your new profile folder.
4. Copy this repository's `user-override.js` to your new profile folder.
...
3. Copy this repository's `thunderbird/user-overrides.js` and `thunderbird/updater.sh` to your profile folder.
...
Note that `thunderbird/user-overrides.js` includes a section with four privacy-breaking preferences that are necessary to authenticate into email accounts using Oauth2. They can and should be deleted after authentication is finished. If you do not need to authenticate into such an account, you can delete them right away.
